...
export const postAppointment = catchAsyncErrors(async (req, res, next) => {
    ...
    const isConflict = await User.findOne({ firstName: doctor_firstName, lastName: doctor_lastName, role: 'Doctor', doctorDepartment: department });
    if (isConflict.length === 0) {
        return next(new ErrorHandler("Doctor not found !", 404));
    }
    if (isConflict.length > 1) {
        return next(new ErrorHandler("Doctor Appointment conflict, please contact through email or phone !", 404));
    }
